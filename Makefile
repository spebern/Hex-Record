# This Makefile is for the Hex::Record extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Hex/Record.pm]
...
#     NAME => q[Hex::Record]
#     PL_FILES => {  }
#     PREREQ_PM => { Storable=>q[0], Test::More=>q[0], Test::Warn=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Hex/Record.pm]
#     clean => { FILES=>q[Hex-Record-*] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Hex/Record.pm \
	lib/Hex/Record/Parser.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Hex
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Hex
...
TO_INST_PM = lib/Hex/Record.pm \
	lib/Hex/Record/Parser.pm

PM_TO_BLIB = lib/Hex/Record.pm \
	blib/lib/Hex/Record.pm \
	lib/Hex/Record/Parser.pm \
	blib/lib/Hex/Record/Parser.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/Hex/Record.pm $(INST_MAN3DIR)/Hex::Record.$(MAN3EXT) \
	  lib/Hex/Record/Parser.pm $(INST_MAN3DIR)/Hex::Record::Parser.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''manipulate intel and srec hex records'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''spebern <bernhard@specht.net>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Hex-Record' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "manipulate intel and srec hex records",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "spebern <bernhard@specht.net>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Hex-Record",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>manipulate intel and srec hex records</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>spebern &lt;bernhard@specht.net&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Storable::" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Warn" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Hex/Record.pm blib/lib/Hex/Record.pm \
	  lib/Hex/Record/Parser.pm blib/lib/Hex/Record/Parser.pm 
